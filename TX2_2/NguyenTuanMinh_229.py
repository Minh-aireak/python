...
class QuanLyBanHang:
    ...
    default_gia_ban = 0  # @staticmethod tương đương
    ...
    def tinh_gia_ban(self):
        self.df["gia_ban"] = self.df.apply(
            lambda r: r["don_gia"] * (1 - r["tyle_giam"]) 
            if r["loai"] == "khuyen_mai" else r["gia_ban"],
            axis = 1,
        )
    ...
    def don_hang_theo_gia_tri(self):
        """Sắp xếp đơn hàng theo thành tiền giảm dần."""
...
if __name__ == "__main__":
    ...
    bao_cao = ql.thong_ke()
    print(bao_cao)
    with open(os.path.join(BASE_DIR, "result.txt"), "w", encoding="utf-8") as f:
        f.write(bao_cao)

    print("\n(Da ghi bao cao ra result.txt)")
